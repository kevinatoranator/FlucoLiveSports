	...
	while($row = $query->fetchObject()){
		//if($row->game_date == $date){
			...
			if($sport=="gsoccer" or  $sport=="bsoccer" or $sport=="jvgsoccer" or $sport=="jvbsoccer"){
				printf("<form action='./manager/soccermanager.php' method='get'><input type ='hidden' id='gameID' name='gameID' value='%s'><input type ='hidden' id='district' name='district' value='true'><input type='submit' class='gameID' value='%s\n%s vs. %s \n@%s'></form><br>", $row->gameID, $row->formattedName, $row->home, $row->away, $row->date);
			}else if($sport=="softball" or  $sport=="baseball" or $sport=="jvsoftball" or $sport=="jvbaseball"){
				printf("<form action='./manager/ballmanager.php' method='get'><input type ='hidden' id='gameID' name='gameID' value='%s'><input type ='hidden' id='district' name='district' value='true'><input type='submit' class='gameID' value='%s\n%s vs. %s \n@%s'></form><br>", $row->gameID, $row->formattedName, $row->home, $row->away, $row->date);
			}else if($sport=="blax" or  $sport=="jvblax"){
				printf("<form action='./manager/blaxmanager.php' method='get'><input type ='hidden' id='gameID' name='gameID' value='%s'><input type ='hidden' id='district' name='district' value='true'><input type='submit' class='gameID' value='%s\n%s vs. %s \n@%s'></form><br>", $row->gameID, $row->formattedName, $row->home, $row->away, $row->date);
