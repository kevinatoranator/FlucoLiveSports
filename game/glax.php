...
<?php
	...
	$sqlsport = "SELECT
		gl.home_quarter1 AS hq1, gl.home_quarter2 AS hq2, gl.home_quarter3 AS hq3, gl.home_quarter4 AS hq4, gl.home_ot AS hot, gl.home_total AS ht, 
		gl.away_quarter1 AS aq1, gl.away_quarter2 AS aq2, gl.away_quarter3 AS aq3, gl.away_quarter4 AS aq4, gl.away_ot AS aot, gl.away_total AS at, gl.completed AS cmp
	...
	*/
	...
	$sql = "SELECT pbp.id AS pbpID, pbp.text AS text, pbp.quarter AS quarter, pbp.time AS tme FROM glax_pbp AS pbp JOIN schedule AS s ON pbp.game_id=s.id WHERE pbp.game_id = '$gameID'";
	$query = $db->prepare($sql);
	$query->execute();
	...
	while($row = $query->fetchObject()){
		...
		$pbpArray[$quarter][] = $pbpText;
	
	}
	
	if($comp == 1){
		for($i = 0; $i<count($pbpArray); $i++){
			if(count($pbpArray[$i]) > 1){
	...
	*/
	...
	$sql = "SELECT rp.number AS num, rp.name AS name, gs.goals AS goals, gs.assists AS assists, gs.ground_balls AS ground_balls, gs.draw_control AS draw_control, gs.forced_turnovers AS forced_turnovers, gs.shots_on_goal AS shots_on_goal,
	gs.turnovers AS turnovers, gs.shots_off_target AS shots_off_target, gs.yellow_cards AS yellow_cards, gs.red_cards AS red_cards, gs.saves AS saves, gs.goals_allowed AS goals_allowed 
	FROM glax_stats AS gs JOIN schedule AS s ON gs.game=s.id JOIN roster_player AS rp ON gs.player=rp.id JOIN roster_teams AS t ON s.team_id=t.id JOIN roster_schools h ON rp.school=h.id
	WHERE gs.game = '$gameID' AND h.short_name = '$homeTeam' AND t.urlName = '$sport' ORDER BY cast(num as unsigned)";
	$query = $db->prepare($sql);
	$query->execute();
	
	$statArray = array(array(["Players"], ["", "Goals", "Assts", "GB", "DC", "SOff", "SOG", "FTO", "TO"]), array(["Goalies"], ["" , "", "", "", "", "", "Saves", "G/A", "Save%"]));
	...
	while($row = $query->fetchObject()){
		...
		$name = implode(" ", $name);
		if($goals != 0 or $assists != 0 or $ground_balls != 0 or $shots_on_goal != 0 or $draw_control != 0 or $forced_turnovers != 0 or $forced_turnovers != 0 or $shots_off_target != 0 or $turnovers != 0){
			$statArray[0][] = [$num . ' ' . $name, $goals, $assists, $ground_balls, $draw_control, $shots_off_target, $shots_on_goal, $forced_turnovers, $turnovers];
		}
	...
	$sql = "SELECT rp.number AS num, rp.name AS name, gs.goals AS goals, gs.assists AS assists, gs.ground_balls AS ground_balls, gs.draw_control AS draw_control, gs.forced_turnovers AS forced_turnovers, gs.shots_on_goal AS shots_on_goal,
	gs.turnovers AS turnovers, gs.shots_off_target AS shots_off_target, gs.yellow_cards AS yellow_cards, gs.red_cards AS red_cards, gs.saves AS saves, gs.goals_allowed AS goals_allowed 
	FROM glax_stats AS gs JOIN schedule AS s ON gs.game=s.id JOIN roster_player AS rp ON gs.player=rp.id JOIN roster_teams AS t ON s.team_id=t.id JOIN roster_schools a ON rp.school=a.id
	WHERE gs.game = '$gameID' AND a.short_name = '$awayTeam' AND t.urlName = '$sport' ORDER BY cast(num as unsigned)";
	$query = $db->prepare($sql);
	$query->execute();
	
	$awaystatArray = array(array(["Players"], ["", "Goals", "Assts", "GB", "DC", "SOff", "SOG", "FTO", "TO"]), array(["Goalies"], ["" , "", "", "", "", "", "Saves", "G/A", "Save%"]));
	//no red or yellow cards
	printf("STATS<hr>");
	...
	while($row = $query->fetchObject()){
		...
		$name = implode(" ", $name);
		if($goals != 0 or $assists != 0 or $ground_balls != 0 or $shots_on_goal != 0 or $draw_control != 0 or $forced_turnovers != 0 or $forced_turnovers != 0 or $shots_off_target != 0 or $turnovers != 0){
			$awaystatArray[0][] = [$num . ' ' . $name, $goals, $assists, $ground_balls, $draw_control, $shots_off_target, $shots_on_goal, $forced_turnovers, $turnovers];
		}
		if($saves != 0 or $goals_allowed != 0){
